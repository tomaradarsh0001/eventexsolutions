...
<div class="edit-container">
    ...
    <div class="header-section">
        <div>
            <h1 class="header-title text-white">Edit Why Us Section</h1>
            <p class="header-subtitle">Update your "Why Choose Us" content to showcase your unique value proposition</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.whyus.update', $whyus->id) }}" method="POST" id="whyusForm">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <textarea 
                        ...
                        rows="4"
                    >{{ old('whyus_paragraph', $whyus->whyus_paragraph) }}</textarea>
        ...
        <div class="card">
            <div class="card-header">
                <h3>
                    ...
                    Why Us Items
                    <span class="existing-badge">{{ $whyus->items->count() }} Items</span>
                </h3>
            </div>
            <div class="card-body">
                <div class="help-text mb-3">
                    <i class="lni lni-bulb"></i>
                    Manage up to 6 items that highlight your key strengths and benefits. Each item will be displayed with an icon, title, and description.
                </div>
                ...
                <div id="itemsContainer">
                    <div class="items-grid" id="itemsGrid">
                        @php
                            ...
                            // Use existing items if no old data, otherwise use old data
                            $displayItems = !empty($oldItems) ? $oldItems : $existingItems->toArray();
                        @endphp
                        
                        @foreach($displayItems as $index => $item)
                        <div class="item-card" data-index="{{ $index }}" data-item-id="{{ $item['id'] ?? '' }}">
                            <div class="item-header">
                                <span class="serial-number" id="serial-{{ $index }}">#{{ sprintf('%02d', $index + 1) }}</span>
                                <button type="button" class="remove-item" onclick="removeItem(this)" {{ $index == 0 && count($displayItems) == 1 ? 'disabled style="opacity:0.5; cursor:not-allowed;"' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <div class="icon-input-group">
                                    <input 
                                        ...
                                        class="form-control icon-input @error('items.'.$index.'.icon') is-invalid @enderror"
                                        placeholder="e.g., lni lni-timer"
                                        value="{{ old('items.'.$index.'.icon', $item['icon'] ?? '') }}"
                                        oninput="updateIconPreview(this)"
                                    >
                                    <div class="icon-preview">
                                        <i class="{{ old('items.'.$index.'.icon', $item['icon'] ?? '') }}"></i>
                            ...
                            <div class="form-group">
                                ...
                                <input 
                                    ...
                                    class="form-control @error('items.'.$index.'.title') is-invalid @enderror"
                ...
                <div class="help-text mt-2 text-center">
                    Maximum 6 items allowed ({{ $whyus->items->count() }}/6 used)
                </div>
            </div>
        </div>
...
@push('scripts')
<script>
    let itemCount = {{ count($displayItems) }};
    const MAX_ITEMS = 6;
    let nextItemId = {{ $whyus->items->count() + 100 }};
    ...
    function removeItem(button) {
        ...
        if (itemCard) {
            ...
            // Add a hidden input to mark item for deletion if it has an ID (existing item)
            ...
            if (itemId) {
                ...
                document.getElementById('whyusForm').appendChild(deleteInput);
            }
            ...
            // Show add button if under max items
            if (itemCount < MAX_ITEMS) {
                const addButton = document.querySelector('.add-item-btn');
                if (addButton) {
    ...
    function updateSerialNumbers() {
        ...
        items.forEach((item, index) => {
            ...
            inputs.forEach(input => {
                ...
                if (name && !name.includes('deleted_items')) {
    ...
    document.getElementById('whyusForm').addEventListener('submit', function(e) {
        ...
        if (hasError) {
            ...
            submitBtn.innerHTML = '<i class="lni lni-spinner lni-spin"></i> Updating...';
            submitBtn.disabled = true;
            document.getElementById('whyusForm').classList.add('loading');
        }
    });
